'use dynamic';
// 📊 十神分析API接口
// GET /api/shen?year=2024&month=1&day=1&hour=0&gender=male&type=solar

import { NextRequest, NextResponse } from 'next/server';
import { calculateBazi } from '../../../lib/bazi';
import { calculateShen } from '../../../lib/shen';
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 计算十神
    const shenAnalysis = calculateShen(bazi);

    if (!shenAnalysis) {
      return NextResponse.json({
        success: false,
        error: 'Failed to calculate shen'
      }, { status: 500 });
    }
    ...
    console.error('Shen calculation error:', error);
    return NextResponse.json({
      success: false,
